...
function image_tag(src, alt) {
  return `<img src="${process.env.MY_ENVIRONMENT === 'production' ? '/docs' : ''}/public/img/${ src }" alt="${alt||'Image'}">`;
}

function logo_image_tag() {
  return `<img src="${process.env.MY_ENVIRONMENT === 'production' ? '/docs' : ''}/public/img/logo.svg" alt="Logo" class="logo" />`;
}
...
module.exports = {
  layout: 'slate.ejs',
  basePath: process.env.MY_ENVIRONMENT === 'production' ? '/docs' : '',
  page_classes: '',
  language_array,
  image_tag,
  logo_image_tag,
